...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <form action="{{ route('admin.kasir.simpan') }}" method="POST" id="kasirForm">
                @csrf
    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                ...
                <div class="cursor-pointer produk-card p-4 border border-slate-100 rounded-lg text-center" data-id="{{ $b->id }}" data-nama="{{ $b->nama_barang }}" data-harga="{{ $b->harga_barang }}" data-stok="{{ $b->stok }}">
                    <div class="w-full h-36 bg-slate-100 rounded-lg mb-3 flex items-center justify-center text-slate-400">
                        <!-- Placeholder image/icon -->
                        <div class="text-center">
                            <i class="fa-solid fa-box-open text-3xl"></i>
                            <div class="text-sm mt-2 font-semibold text-slate-700">{{ $b->nama_barang }}</div>
                        </div>
                    </div>
                    <div class="text-sm text-slate-500">Stok: {{ $b->stok }}</div>
                    <div class="text-lg font-bold text-blue-600">Rp {{ number_format($b->harga_barang,0,',','.') }}</div>
                </div>
                @endforeach
            </div>
        </div>
